...
let Util = {
     ...
     */
    buildUrl(response) {
        let city_cords = '42.3601,-71.0589';

        if (response === undefined || response === '') {
            winston.log('info', `running API call for url ${config.API_SERVER}/forecast/${config.KEY}/${city_cords}`);
            return `${config.API_SERVER}/forecast/${config.KEY}/${city_cords}`
        }
        winston.log('info', `${config.API_SERVER}/forecast/${config.KEY}/${response}`);
    ...
    buildGoogleApiUrl(city) {
        winston.log('info', `running API call getting city cords from google API ${config.GOOGLE_MAP}${city}`);
